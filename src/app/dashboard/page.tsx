...
export default function DashboardPage() {
  ...
  const handleCopyBridgeToken = async () => {
    setBridgeMessage('');
    try {
      const res = await fetch('/api/portal-bridge/token');
      const data = await res.json();
      if (!res.ok || !data.success || !data.token) {
        throw new Error(data.message || 'Không lấy được bridge token');
      }

      await navigator.clipboard.writeText(data.token);
      setBridgeMessage('Đã copy bridge token. Dán token này vào extension UTH Portal Bridge.');
    } catch (err) {
      setBridgeMessage(err instanceof Error ? err.message : 'Không copy được bridge token');
    }
  };
  ...
  return (
    <div className="min-h-screen pro-bg pro-scrollbar">
      ...
      <main className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
        ...
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-2 rounded-lg border border-cyan-400/20 bg-cyan-950/20 p-3 text-cyan-100">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <KeyRound className="h-4 w-4 text-cyan-300 flex-shrink-0" />
            <span className="text-xs sm:text-sm">
              UTH Portal Bridge dùng token này để đồng bộ dữ liệu từ tab portal đã đăng nhập.
            </span>
          </div>
          <Button
            type="button"
            onClick={handleCopyBridgeToken}
            className="bg-cyan-600 hover:bg-cyan-700 text-white h-9 px-3 text-xs sm:text-sm"
          >
        ...
        {bridgeMessage && (
          <div className="mb-4 rounded-lg border border-cyan-400/20 bg-slate-900/60 p-3 text-xs sm:text-sm text-cyan-100">
            {bridgeMessage}
          </div>
        )}
