...
import { Loader2, User, Lock, AlertCircle, CheckCircle, KeyRound } from 'lucide-react';
...
export function LoginForm({ recaptchaSiteKey }: LoginFormProps) {
  ...
  const [uthToken, setUthToken] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === 'token' && !uthToken.trim()) {
      setError('Vui lòng nhập token UTH');
      return;
    }

    if (authMode === 'password' && recaptchaSiteKey && !recaptchaToken) {
    ...
    try {
      const response = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({
          ...
          recaptchaToken: authMode === 'password' ? (recaptchaToken || 'bypass-for-dev') : undefined,
          uthToken: authMode === 'token' ? uthToken.trim() : undefined
        }),
      });
      ...
      if (data.success) {
        ...
        try {
          ...
          const studentName = data.data?.studentName || data.student?.fullName || data.student?.name || '';
      ...
      <CardContent className="p-4 sm:p-6 pt-0 sm:pt-0">
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-2 rounded-md border border-gray-200 bg-gray-50 p-1">
            <button
              type="button"
              className={`rounded px-3 py-2 text-xs sm:text-sm font-medium transition ${
                authMode === 'password' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'
              }`}
              onClick={() => {
            ...
            <button
              type="button"
              className={`rounded px-3 py-2 text-xs sm:text-sm font-medium transition ${
                authMode === 'token' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-600'
              }`}
              onClick={() => {
                setAuthMode('token');
                setError('');
          ...
          {authMode === 'password' ? (
            ...
            <div className="space-y-1.5 sm:space-y-2">
              <label htmlFor="uthToken" className="text-xs sm:text-sm font-medium text-gray-700">
                Token UTH
              </label>
              <div className="relative">
                ...
                <textarea
                  ...
                  value={uthToken}
                  onChange={(e) => setUthToken(e.target.value)}
                  className="min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 pl-10 text-xs ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                  disabled={isLoading}
                  autoComplete="off"
                  spellCheck={false}
                />
          ...
          {/* reCAPTCHA */}
          {authMode === 'password' && recaptchaSiteKey && (
          ...
          >
            {isLoading ? (
              ...
              authMode === 'password' ? 'Đăng nhập' : 'Đăng nhập bằng token'
            )}
          </Button>
