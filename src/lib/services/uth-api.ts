...
function isCloudflareChallenge(response: Response, bodyText: string): boolean {
  return response.headers.get('cf-mitigated') === 'challenge'
    || bodyText.includes('Just a moment...')
    || bodyText.includes('/cdn-cgi/challenge-platform/');
}

function buildNonJsonError(response: Response, bodyText: string): UTHApiError {
  if (isCloudflareChallenge(response, bodyText)) {
    return new UTHApiError(
      'UTH đang chặn request tự động bằng Cloudflare/anti-bot. Hãy thử đăng nhập trực tiếp trên portal.ut.edu.vn hoặc kiểm tra IP/server đang bị challenge.',
      response.status || 403,
  ...
  return new UTHApiError(
    `UTH trả về dữ liệu không phải JSON (HTTP ${response.status}, content-type: ${contentType}). ${preview}`,
    response.status,
    'NON_JSON_RESPONSE'
  );
}

async function readUthJson<T>(response: Response): Promise<T> {
  const bodyText = await response.text();

  let data: T & { message?: string | null; status?: number };
  try {
    data = bodyText ? JSON.parse(bodyText) : null;
  } catch {
    throw buildNonJsonError(response, bodyText);
  }

  if (!response.ok) {
    throw new UTHApiError(
      data?.message || `UTH trả về HTTP ${response.status}: ${response.statusText}`,
...
export class UTHApiService {
  ...
  async login(credentials: LoginRequest): Promise<{ response: LoginResponse; cookies: UTHCookies; token: string }> {
    try {
      ...
      const data = await readUthJson<LoginResponse>(response);
      ...
      throw new UTHApiError(`Lỗi kết nối tới server UTH: ${message}`, 500, 'NETWORK_ERROR');
    }
  }
  ...
  /**
   * Get registration periods and their current registration state
   */
  async getRegistrationPeriods(): Promise<RegistrationPeriod[]> {
    const url = `${this.baseUrl}/dkhp/getDot`;

    const response = await this.authenticatedRequest<RegistrationPeriod[]>(url);
    return response.body || [];
  }

  /**
   * Resolve the currently active registration period.
   */
  async getActiveRegistrationPeriodId(): Promise<number> {
    const periods = await this.getRegistrationPeriods();
    const activePeriod = periods.find((period) => period.isDangKyHocPhan)
      || periods.find((period) => !period.isHanCheDot)
      || periods[0];

    if (!activePeriod) {
      throw new UTHApiError('Không tìm thấy đợt đăng ký học phần', 404, 'NO_REGISTRATION_PERIOD');
    }
  ...
  async getStudentInfo(): Promise<StudentInfo | null> {
    try {
      return await this.getStudentInfoStrict();
    } catch (error) {
  ...
  /**
   * Get student profile and surface auth/upstream errors to callers.
   */
  async getStudentInfoStrict(): Promise<StudentInfo> {
  ...
  private async authenticatedRequest<T>(
    ...
    const response = await retryAsync(async () => {
      ...
      if (!res.ok) {
        await readUthJson<ApiResponse<T>>(res);
      }

      return res;
    }, 3, 1000);

    const data = await readUthJson<ApiResponse<T>>(response);
...
export function getUTHApi(cookies?: UTHCookies, token?: string): UTHApiService {
  return new UTHApiService(cookies, token);
}
