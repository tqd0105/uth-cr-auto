...
import { RegistrationSchedule, RegistrationLog, UserConfig, RegistrationStatus, Donation, DonationStatus, PortalBridgeSnapshot } from './types/uth';
...
export async function initDatabase() {
  try {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS portal_bridge_snapshots (
    ...
    `;
    await sql`CREATE INDEX IF NOT EXISTS idx_portal_bridge_student ON portal_bridge_snapshots(student_id)`;
...
async function ensurePortalBridgeTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS portal_bridge_snapshots (
      user_session TEXT PRIMARY KEY,
      student_id TEXT,
...
export const portalBridgeDb = {
  upsert: async (userSession: string, studentId: string | null, snapshot: PortalBridgeSnapshot) => {
    await ensurePortalBridgeTable();
    const payload = JSON.stringify(snapshot);
    const result = await sql`
      INSERT INTO portal_bridge_snapshots (user_session, student_id, payload, synced_at, updated_at)
      VALUES (${userSession}, ${studentId}, ${payload}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      ON CONFLICT (user_session) DO UPDATE SET
  ...
  findBySession: async (userSession: string): Promise<PortalBridgeSnapshot | null> => {
    await ensurePortalBridgeTable();
    const result = await sql`
      SELECT payload FROM portal_bridge_snapshots WHERE user_session = ${userSession}
    `;
    ...
    try {
      return JSON.parse(payload) as PortalBridgeSnapshot;
    } catch {
      return null;
    }
  }
